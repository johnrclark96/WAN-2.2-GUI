#!/usr/bin/env python
# wan22_webui_a1111.py
# A1111-style Gradio UI adapted for WAN 2.2 runner.
...
def normalize_lora_table(lora_rows):
    """
    Accepts: None, list-of-lists, list-of-dicts, or a pandas.DataFrame.
    ...
    try:
        ...
        if isinstance(lora_rows, _pd.DataFrame):
...
def ingest_loras(files, lora_dir: Path) -> List[List]:
    """
    Copies LoRAs into lora_dir and returns table rows [[path, weight], ...].
    Handles both path strings and file objects. Skips non-LoRA extensions.
    """
    rows = []
...
def build_cmd(
    ...
    for row in rows:
        path = str(row[0]).strip() if len(row) > 0 else ""
        if not path:
            continue
        try:
    ...
    # Extra args (free-form)
    if extra:
        try:
            cmd += shlex.split(extra)
        except Exception:
            cmd += [extra]  # last resort
...
def stream_run(cmd: List[str], outdir: Path):
    """
    Generator that runs the process and yields (logs, video_path, gen_info_dict).
    """
    global PROC
    logs = ""
    video = None
    ...
    # First yield so UI draws immediately
    ...
    try:
        ...
        logs += f"\n[ERROR] {e}\nCheck the 'Runner path' and that Python can start it.\n"
        ...
        if not video and outdir.exists():
            ...
            for ext in (".mp4", ".gif", ".webm", ".mov"):
                for p in outdir.rglob(f"*{ext}"):
                    try:
                        t = p.stat().st_mtime
                    except Exception:
                        continue
                    if t >= start and t > tmax:
                        newest, tmax = p, t
            if newest:
        ...
        with gr.Tabs():
            ...
            with gr.Tab("txt2vid"):
                with gr.Row():
                    with gr.Column(scale=6, elem_id="left-col"):
                        with gr.Row():
                            model_sel = gr.Textbox(label="WAN Checkpoint", value="wan2.2 (auto)", interactive=False)
                        ...
                        with gr.Row():
                            ...
                            recycle = gr.Button("♻️", elem_classes=["smallbtn"])

                        gr.Markdown("**LoRAs** (drop files or edit below):")
                        ...
                        with gr.Accordion("Runner / Output", open=False):
                            ...
                            extra = gr.Textbox(label="Extra args", placeholder="--vae path\\to\\vae.safetensors ...")

                    with gr.Column(scale=5, elem_id="right-col"):
                        result_video = gr.Video(label="Result")
                        gen_info = gr.JSON(label="Generation Info")
                        console = gr.Textbox(label="Console", lines=20)
                ...
                def do_generate_txt(p, n, samp, st, w, h, f_fps, f_frames, bcnt, bsz, cfg_s, seed_s, loras_tbl, runner_p, out_dir, extra_flags):
                    # Validate early
                    if not (p or "").strip():
                        raise gr.Error("Prompt is required for txt2vid.")
                    ...
                    # STREAM: make this function a generator
                    yield from stream_run(cmd, Path(out_dir))

                generate.click(
                    do_generate_txt,
                    inputs=[prompt, neg, sampler, steps, width, height, fps, frames, batch_count, batch_size, cfg, seed, t_loras, runner, outdir, extra],
            ...
            with gr.Tab("img2vid"):
                with gr.Row():
                    with gr.Column(scale=6, elem_id="left-col"):
                        with gr.Row():
                            model_sel2 = gr.Textbox(label="WAN Checkpoint", value="wan2.2 (auto)", interactive=False)
                            style_dd2 = gr.Dropdown(choices=list(STYLES.keys()), value="None", label="Styles")

                        with gr.Row():
                            prompt2 = gr.Textbox(label="Prompt", lines=4)
                            with gr.Column(scale=1):
                    ...
                    with gr.Column(scale=5, elem_id="right-col"):
                        ...
                        console2 = gr.Textbox(label="Console", lines=20)

                def on_style2(style, current_neg):
                    return STYLES.get(style, {}).get("neg", current_neg)

                style_dd2.change(on_style2, inputs=[style_dd2, neg2], outputs=[neg2])

                def on_add_loras_img(files, current_table):
                    existing = normalize_lora_table(current_table)
                    new_rows = ingest_loras(files, DEFAULT_LORA_DIR)
                    return existing + new_rows

                add_lora_btn2.click(on_add_loras_img, inputs=[i_files, i_loras], outputs=[i_loras])
                ...
                def do_generate_img(p, n, init, samp, st, w, h, f_fps, f_frames, bcnt, bsz, cfg_s, seed_s, loras_tbl, runner_p, out_dir, extra_flags):
                    # Validate
                ...
                generate2.click(
                    do_generate_img,
                    inputs=[prompt2, neg2, init_img, sampler2, steps2, width2, height2, fps2, frames2, batch_count2, batch_size2, cfg2, seed2, i_loras, runner2, outdir2, extra2],
            ...
            # ===================== CONSOLE =====================
            with gr.Tab("Console"):
                gr.Markdown("Use the per-tab consoles; this is a placeholder for future global logs.")
...
def main():
    ...
    ap.add_argument("--auth", type=str, default=None, help="user:pass")
    args = ap.parse_args()

    ui = build_ui()
    auth = tuple(args.auth.split(":", 1)) if args.auth and ":" in args.auth else None

    # --- auto fallback: 7860 -> 7861 if busy (e.g., A1111)
    ...
    if args.port == 7860 and is_port_in_use(7860):
        if not is_port_in_use(7861):
            ...
            print("[ui] Ports 7860 and 7861 are busy; using requested 7860 and letting Gradio handle the error.",
                  flush=True)
