...
def main():
    ...
    for v in call_variants:
        try:
            kwargs = {**common_kwargs, **v}
            log(f"[ps1-engine] Generatingâ€¦ steps={kwargs.get('num_inference_steps')} frames={kwargs.get('num_frames') or kwargs.get('video_length')} fps={kwargs.get('fps')}")
            result = pipe(**kwargs)
            break  # success
